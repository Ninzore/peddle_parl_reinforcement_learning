...
ACTOR_LR = 5e-4   # Actor网络更新的 learning rate
CRITIC_LR = 5e-3   # Critic网络更新的 learning rate

GAMMA = 0.99        # reward 的衰减因子，一般取 0.9 到 0.999 不等
TAU = 0.001         # target_model 跟 model 同步参数 的 软更新参数
MEMORY_SIZE = 1e6   # replay memory的大小，越大越占用内存
MEMORY_WARMUP_SIZE = 1.2e4      # replay_memory 里需要预存一些经验数据，再从里面sample一个batch的经验让agent去learn
...
def run_episode(env, agent, rpm):
    ...
    while True:
        ...
        mean = np.mean(action)
        action = mean + (mean - action)/5
        action = np.clip(action, 0.1, 15)
...
ckpt = 'model_dir/steps_180193.ckpt'
agent.restore(ckpt)
